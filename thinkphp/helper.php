...
if (!function_exists('lang')) {
    ...
    {
        //判斷語系載入來源為前台還是後臺，根據入口檔案內的ENTRANCE定義值來設定前後台
        if (ENTRANCE == 'index') {
            $s_def_lang = cookie('platform_lang') != '' ? cookie('platform_lang') : $GLOBALS['config']['site']['lang'];
        } else {
            $s_def_lang =  empty($GLOBALS['config']['app']['lang']) ? null : $GLOBALS['config']['app']['lang'];
        }
        //判斷使用者有無更改語系，無更改則跑系統預設
        $lang = $lang == '' ? $s_def_lang : $lang;
        return Lang::get($name, $vars, $lang);
    }
}
/* leo 2021/12/09 前台語系預設 */
if (!function_exists('platform_lang')) {
    /**
     * 获取语言变量值
     * @param string    $name 语言变量名
     * @param array     $vars 动态变量值
     ...
     * @return mixed
     */
    function platform_lang($name, $vars = [], $lang = '')
    {
        //判斷使用者有無更改語系，無更改則跑系統預設
        $s_def_lang = cookie('platform_lang') != '' ? cookie('platform_lang') : $GLOBALS['config']['site']['lang'];
        //可自行在$lang定義需要哪種語系，若無則跑預設
        $lang = $lang == '' ? $s_def_lang : $lang;
