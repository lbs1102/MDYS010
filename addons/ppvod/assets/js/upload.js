                ...
                (new WebUploader.Uploader()).md5File(file, 0, 40 * 1024 * 1024).progress(function (percentage) {
                    ...
                    $.ajax({
                        ...
                        if (data.ifExist) { //若存在，这返回失败给WebUploader，表明该文件不需要上传
                            ...
                            //alert(JSON.stringify(data));
                            if (data.share) {
                                $("#vod_name").val(data.title);
                                $("#vod_pic").val(data.pic);
                                $("#vod_duration").val(data.duration);

                                var src = data.url;
                                src = urlClass == 'm3u8' ? data.url : data.share;
                                var pic_screenshot = $("#vod_pic_screenshot")
                                var content = $("#vod_content1");
                                if(content.val() ==''){
                                   content.val(data.title+'$'+src);
                                }else{
                                   content.val(content.val()+'\r\n'+data.title+'$'+src);
                                };
                                //截图
                                if(pic_screenshot.val() ==''){
                                   pic_screenshot.val(data.pic)
                                }else{
                                   pic_screenshot.val(pic_screenshot.val()+'\r\n'+data.pic);
                                }
                                $('.screenshot_list').append('<div data-src="'+data.pic+'"><a href="javascript:;" class="del_screenshot">删除</a><img src="'+mac_url_img(data.pic)+'" alt="" class="layui-upload-img screenshot-img"></div>');
                                 
 
								$("select option[value='ppvod']").attr("selected","selected");
								$('#player_list .layui-form-item').children("div").children("div").children("div").children("input").eq(0).val('ppvod官方播放器');
                           ...
                           if (data.share) {
                                ...
                                $("#vod_duration").val(data.duration);
                                 ...
                                 if(content.val() ==''){
                                    ...
                                    var addsrc = content.val()+'\r\n'
                                    content.val(addsrc+data.title+'$'+src);
                                 };
                                 if(pic_screenshot.val() ==''){
                                    pic_screenshot.val(data.pic)
                                 }else{
                                    pic_screenshot.val(pic_screenshot.val()+'\r\n'+data.pic);
								         ...
								         $("select option[value='"+player_name+"']").attr("selected","selected");
								         $('#player_list .layui-form-item').children("div").children("div").children("div").children("input").eq(0).val(player_show);
                                var id = data.share.substring(data.share.lastIndexOf("/") + 1, data.share.length);
                                $("#vid").val(id);
                                $("#erweima").val(data.qr);
                            } else {
