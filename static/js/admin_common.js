...
layui.define(["element", "form"], function (exports) {
  ...
  var lockscreen = function () {
    ...
    layer.open({
      ...
      type: 1,
      content:
        '<div class="lock-screen"><input type="password" class="unlockedPwd layui-input" placeholder="请输入登录密码解锁..." autocomplete="off"><button class="unlocked layui-btn">解锁</button><div class="unlockTips"></div></div>',
      closeBtn: 0,
      shade: 0.95,
      offset: "350px",
    });
  };
  var admin_edit = function (page) {
    // document.oncontextmenu=new Function("event.returnValue=false;");
    // document.onselectstart=new Function("event.returnValue=false;");
    layer.open({
      title: false,
  ...
  /* leo 2022/03/21 更改登入管理者相關設定頁面 */
  $("#admin_edit").click(function (elem) {
    elem.preventDefault();
  ...
  $(document).on("click", ".unlocked", function () {
    ...
    $.post(
      ADMIN_PATH + "/admin/index/unlocked",
      { password: pwd },
      function (res) {
        if (res.code == 1) {
  ...
  if (window.localStorage.getItem("adminNavTag")) {
    $('#switchNav a[href="' + window.localStorage.getItem("adminNavTag") + '"]')
      .parent("dd")
      .addClass("layui-this")
      .parents("li")
        ...
        .hover(
          function () {
            if (that.hasClass("close")) {
              ...
              $('a[href="' + window.localStorage.getItem("adminNavTag") + '"]')
                .parent("dd")
                .addClass("layui-this")
                .parents("li")
                .addClass("layui-nav-itemed")
  ...
  $("#fullscreen-btn").click(function () {
    ...
    if (!that.hasClass("ai-quanping")) {
      ...
      that
        .addClass("ai-quanping")
        .removeClass("ai-quanping1")
        .parents(".page-body")
        .addClass("fullscreen");
      $(".page-tab-content").css({
        "min-height": $(window).height() - 63 + "px",
      });
    } else {
      $("#switchBody").css({ "z-index": 998 });
      $("#switchNav").css({ "z-index": 1000 });
      that
        .addClass("ai-quanping1")
        .removeClass("ai-quanping")
        .parents(".page-body")
  ...
  $(document).on("click", ".j-iframe", function () {
    ...
    if (_checkbox) {
      ...
      _ids = ids.join(",");
      _url =
        _url.indexOf("?") > -1 ? _url + "&ids=" + _ids : _url + "?ids=" + _ids;
    }
    var lay = layer.open({
      type: 2,
      title: _title,
