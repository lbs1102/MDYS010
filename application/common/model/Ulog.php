...
class Ulog extends Base
{
    ...
    public function listData($where, $order, $page = 1, $limit = 20, $start = 0)
    {
        ...
        foreach ($list as $k => &$v) {
            ...
            if ($v['ulog_mid'] == 1) {
                ...
                // }
                //leo 2022/04/28 取得當前前台語系
                $s_platform_lang = is_null(cookie('platform_lang')) ? $GLOBALS['config']['site']['lang'] : cookie('platform_lang');

                $vod_name = "";
                switch ($s_platform_lang) {
                    case 'vi-vn':
                        $vod_name = $vod_info['info']['vod_vi'];
                        break;
                    case 'en-us':
                        $vod_name = $vod_info['info']['vod_en'];
                        break;
                    default:
                        $vod_name = $vod_info['info']['vod_name'];
                        break;
                }
    ...
    public function saveData($data)
    {
        ...
        return ['code' => 1, 'msg' => lang('save_ok'), 'info' => ["ulog_id" => $res]];
    }
