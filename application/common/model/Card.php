...
class Card extends Base {
    ...
    public function saveAllData($num,$money,$point,$role_no)
    {
        $data=[];
        $t = time();
        for($i=1;$i<=$num;$i++){
            ...
            $data[$card_no] = ['card_no'=>$card_no,'card_money'=>$money,'card_points'=>$point,'card_add_time'=>$t];
        }
    ...
    public function useData($card_no,$user_info)
    {
        if (empty($card_no) || empty($user_info)) {
            return ['code' => 1001, 'msg'=>lang('param_err')];
        }
