        ...
        foreach ($data['data'] as $k => $v) {
            ...
            if ($v['type_id'] == 0) {
                ...
                foreach ($cj_play_from_arr as $kk => $vv) {
                    ...
                    if (empty($players[$vv])) {
                        /*leo 若資料指定的撥放器未設定則預設 dplayer*/
                        $cj_play_from_arr[$kk] = "dplayer";
                ...
                if (!$info) {
                    ...
                    if ($param['opt'] == 2) {
                        ...
                        if (config('default_platform_switch')) {
                            ...
                            $res = json_decode(mac_curl_ajax("POST", 'Vod', $ajax_arr), true);
                            if ($res["code"] == 1) {
                                $res = true;
                            } else {
                                $this->error = $res['msg'];
                    ...
                    if (empty($config['uprule'])) {
                        ...
                        if (strpos(',' . $config['uprule'], 'j') !== false && !empty($v['vod_pic'])) {
                            $tmp = $this->syncImages($config['pic'], $v['vod_pic'], 'vod');
                            $update['vod_pic'] = (string)$tmp['pic'];
                            $update['vod_pic_thumb'] = $update['vod_pic'];
                            $msg = $tmp['msg'];
                            $path = './';
                            $pic = $path . $info['vod_pic'];
                            if (file_exists($pic)) {
                                unlink($pic);
                            }
                            $pic = $path . $info['vod_pic_thumb'];
                            if (file_exists($pic)) {
                                unlink($pic);
                            }
                        }
                        ...
                        if (count($update) > 0) {
                            ...
                            if (config('default_platform_switch')) {
                                $ajax_arr = [];
                                $ajax_arr['vod_id'] = $where['vod_id'];
                                foreach ($update as $key => $val) {
                                    $ajax_arr[$key] = $val;
                                }
                                $res = json_decode(mac_curl_ajax("PUT", 'Vod', $ajax_arr), true);
                                if ($res["code"] == 1) {
                                    $res = true;
                                } else {
                                    $this->error = $res['msg'];
