...
class User extends Base
{
    ...
    public function saveData($data)
    {
        ...
        if (!empty($data['user_id'])) {
            ...
            if (config('default_platform_switch')) {
                $ajax_arr = [];
                $ajax_arr["user_id"] = (int)$data['user_id'];
                foreach ($data as $k => $v) {
                    $ajax_arr[$k] = $v;
                }
                $res = json_decode(mac_curl_ajax("PUT", 'User', $ajax_arr), true);
                if ($res["code"] == 1) {
                    $res = true;
                } else {
                    $this->error = $res['msg'];
    ...
    public function register($param)
    {
        ...
        if (
            empty($param['user_openid_qq']) && empty($param['user_openid_weixin'])
        ...
        if (mac_not_only_num_en($data['user_name'])) {
            return ['code' => 1006, 'msg' => lang('model/user/name_contain')];
        }

        if (strlen($data['user_pwd']) < 5) {
            return ['code' => 1014, 'msg' => lang('model/user/pass_length_err')];
        }
        if (mac_not_only_num_en($data['user_pwd'])) {
            return ['code' => 1013, 'msg' => lang('model/user/pass_contain')];
        }
    ...
    public function info($param)
    {
        ...
        if (empty($param['user_pwd1']) || empty($param['user_pwd2'])) {
            return ['code' => 1004, 'msg' => lang('model/user/new_pass_err')];
        }
        if (strlen($param['user_pwd1']) < 5) {
            return ['code' => 1006, 'msg' => lang('model/user/pass_length_err')];
        }
        if (mac_not_only_num_en($param['user_pwd'])) {
            return ['code' => 1005, 'msg' => lang('model/user/pass_contain')];
        }
        if (mac_not_only_num_en($param['user_pwd1'])) {
            return ['code' => 1005, 'msg' => lang('model/user/pass_contain')];
        }
        if (mac_not_only_num_en($param['user_pwd2'])) {
            return ['code' => 1005, 'msg' => lang('model/user/pass_contain')];
        }
        $data = [];
    ...
    public function login($param)
    {
        ...
        if (config('default_platform_switch')) {
            ...
            $res = json_decode(mac_curl_ajax("GET", 'User', $ajax_arr), true);
            $row = $res["data"][0];
        } else {
            $row = $this->where($where)->find();
        }
        ...
        $ip = (int) sprintf('%u', ip2long(request()->ip()));
        if ($ip > 2147483647) {
            $ip = 0;
        }
    ...
    public function upgrade($param)
    {
        ...
        $long = $param['long'];
        $points_long = [
            'day' => 86400 * $GLOBALS['config']['site']['site_time_0'],
            'week' => 86400 * $GLOBALS['config']['site']['site_time_1'],
            'month' => 86400 * $GLOBALS['config']['site']['site_time_2'],
            'year' => 86400 * $GLOBALS['config']['site']['site_time_3']
        ];

        if (!array_key_exists($long, $points_long)) {
            return ['code' => 1001, 'msg' => lang('model/user/illegal_operation')];
        }
        ...
        if (config('default_platform_switch')) {
            $ajax_arr = [];
            $ajax_arr['user_id'] = $where['user_id'];
            foreach ($data as $k => $v) {
                $ajax_arr[$k] = $v;
            }
            $res = json_decode(mac_curl_ajax("PUT", 'User', $ajax_arr), true);
    ...
    public function findpass_reset($param)
    {
        ...
        if (mac_not_only_num_en($param['user_pwd'])) {
            return ['code' => 2010, 'msg' => lang('model/user/pass_contain')];
        }
        if (mac_not_only_num_en($param['user_pwd2'])) {
            return ['code' => 2010, 'msg' => lang('model/user/pass_contain')];
        }
