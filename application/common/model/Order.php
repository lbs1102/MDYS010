...
class Order extends Base
{
    ...
    public function saveData($data)
    {
        ...
        if (!empty($data['order_id'])) {
            ...
            if (config('default_platform_switch')) {
                $ajax_arr = [];
                $ajax_arr['order_id'] = $data['order_id'];
                foreach ($data as $k => $v) {
                    $ajax_arr[$k] = $v;
                }
                $res = json_decode(mac_curl_ajax("PUT", 'Order', $ajax_arr), true);
                if ($res["code"] == 1) {
                    $res = true;
                } else {
                    $this->error = $res['msg'];
            ...
            if (config('default_platform_switch')) {
                ...
                $res = json_decode(mac_curl_ajax("POST", 'Order', $ajax_arr), true);
                if ($res["code"] == 1) {
                    $res = true;
                } else {
                    $this->error = $res['msg'];
    ...
    public function notify($order_code, $pay_type)
    {
        ...
        if ($order != "") {
            $group_dat_setting = trim($order['info']['order_remarks']);
            foreach ($group_day_list as $k => $v) {
                if ($k == $group_dat_setting) {
                    $order_add_time = ((int) $v) * 86400;
                }
            }
        }
        $where2 = [];
        ...
        if (isset($order_add_time)) {
            if ($user['info']['user_end_time'] > time()) {
                $end_time = ((int) $user['info']['user_end_time']) + $order_add_time;
            } else {
                $end_time = time() + $order_add_time;
            }
            $res = model('User')->where($where2)->update(['user_end_time' => $end_time]);
        } else {
