...
class Vod extends Base
{
        ...
        foreach ($list as $k => $v) {
            ...
            if (ENTRANCE == "index") {
                ...
                switch ($s_platform_lang) {
                    case 'vi-vn':
                        $list[$k]['vod_name'] = $v['vod_vi'];
                        break;
                    case 'en-us':
                        $list[$k]['vod_name'] = $v['vod_en'];
                        break;
                }

                $list[$k]['vod_name'] = str_replace("\"", "&quot;", $list[$k]['vod_name']);
                $list[$k]['vod_name'] = str_replace("'", "&apos;", $list[$k]['vod_name']);
            }
        }
    ...
    public function infoData($where, $field = '*', $cache = 0)
    {
        ...
        if ($GLOBALS['config']['app']['cache_core'] == 0 || $cache == 0 || empty($info['vod_id'])) {
            ...
            if (config('default_platform_switch')) {
                ...
                foreach ($where as $key => $val) {
                    $ajax_arr[$key] = is_array($val) ? $val[1] : $val;
                }
                $res = json_decode(mac_curl_ajax("GET", 'Vod', $ajax_arr), true);
                $info = $res["data"][0];
            } else {
                /* 系統內建API */
        ...
        $info['vod_en'] = str_replace("\"", "&quot;", $info['vod_en']);
        $info['vod_vi'] = str_replace("\"", "&quot;", $info['vod_vi']);
