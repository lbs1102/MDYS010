...
class All extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->_tsp = strtotime("now");
    }
    /** leo 2021/12/20 轉跳功能複寫 **/
    public function success($msg = '', $url = null, $data = '', $wait = 3, array $header = [])
    {
        $active_path = $this->request->pathinfo();
        // $data若不為空字串則為轉跳資料用不須通知
        if (strpos($active_path, "admin/") !== false && $url == null) {
            if (strpos($active_path, "type/extend") == false) {
                mac_send_notify("Active '" . $active_path . "'   " . $msg, "log", $data);
            };
        }
    ...
    public function error($msg = '', $url = null, $data = '', $wait = 3, array $header = [])
    {
        ...
        if (strpos($active_path, "admin/") !== false) {
            mac_send_notify("Active '" . $active_path . "'   " . $msg, "notice", $data);
        }
    ...
    protected function label_maccms()
    {
        ...
        $maccms['lang'] = mac_get_platform_lang();
    ...
    protected function label_user()
    {
        ...
        if ($user['user_id'] > 0) {
            $where = [];
            $where['user_pid'] = ['eq', $user['user_id']];
            $res = model('User')->countData($where);
            $user['user_downline'] = $res;
        }
        $GLOBALS['user'] = $user;
    ...
    /** leo 2021/12/20 - 定時任務網址 **/
    protected function label_timming()
    {
        $config = config('timming');
        $timming_api_path = "api.php/timming/index/"; // timming api 接口
        $timming_url = [];
        foreach ($config as $k => $v) {
            if ($v['status'] == 1) {
                $timming_url[] = $GLOBALS['config']['site']['install_dir'] . $timming_api_path . "?name=" . $k;
            }
        }
        $this->assign('timming_list', $timming_url);
    }
    ...
    protected function label_vod_play($flag = 'play', $info = [], $view = 0, $pe = 0)
    {
        ...
        $res_favs =  $this->check_user_favs($param, $flag);
        if ($res_favs !== false && $res_favs['code'] == 1) {
            $info['favs'] = 1;
            $info['favs_id'] = $res_favs['info']['ulog_id'];
        } else {
            $info['favs'] = 0;
            $info['favs_id'] = 0;
        }
        ...
        //leo 2021/12/15 取得當前前台語系
        $s_platform_lang = is_null(cookie('platform_lang')) ? $GLOBALS['config']['site']['lang'] : cookie('platform_lang');
        /**leo 2021/12/15  語系切換影片名稱 **/
        switch ($s_platform_lang) {
            case 'vi-vn':
                $info['vod_name'] = $info['vod_vi'];
                break;
            case 'en-us':
                $info['vod_name'] = $info['vod_en'];
                break;
        }
        $this->assign('obj', $info);
        ...
        if ($pe == 0 && $flag == 'play' && ($popedom['trysee'] > 0) || ($info['vod_pwd_' . $flag] != '' && session($pwd_key) != '1') || ($info['vod_copyright'] == 1 && !empty($info['vod_jumpurl']) && $GLOBALS['config']['app']['copyright_status'] == 4)) {
            ...
            if ($this->check_vod_mode($info, $flag)) {
                $this->assign('player_data', '<script type="text/javascript">var player_aaaa=' . json_encode($player_info) . '</script>');
                $this->assign('player_js', '<script type="text/javascript" src="' . MAC_PATH . 'static/js/playerconfig.js?t=' . $this->_tsp . '"></script><script type="text/javascript" src="' . MAC_PATH . 'static/js/player.js?ver=' . $this->_tsp . '"></script>');
            } else {
                if ($this->check_user_free_watch($param, $flag)) {
                    $this->assign('player_data', '<script type="text/javascript">var player_aaaa=' . json_encode($player_info) . '</script>');
                    $this->assign('player_js', '<script type="text/javascript" src="' . MAC_PATH . 'static/js/playerconfig.js?t=' . $this->_tsp . '"></script><script type="text/javascript" src="' . MAC_PATH . 'static/js/player.js?ver=' . $this->_tsp . '"></script>');
                } else {
                    $id = $info['vod_id'];
                    ...
                    } elseif ($GLOBALS['config']['rewrite']['vod_id'] == 1) {
                        $id = $info['vod_en'];
                    }
