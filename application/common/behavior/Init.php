...
class Init
{
    public function run(&$params)
    {
        ...
        $_defind_lang = ENTRANCE == 'index' ? $config['site']['lang'] : $config['app']['lang'];
        Lang::range($_defind_lang);

        // leo 2021/12/09 自定義語言包加載修改
        $a_lang = scandir(APP_PATH . 'lang');
        foreach ($a_lang as $k => $v) {
            if (strpos($v, EXT)) {
                $s_lang_dir = APP_PATH . 'lang' . DS . $v;
                $s_lang = explode(".", $v)[0];
                Lang::load($s_lang_dir, $s_lang);
            }
        }
        ...
        if (!empty($config['site']['lang'])) {
            config('default_platform_lang', $config['site']['lang']);
        }
