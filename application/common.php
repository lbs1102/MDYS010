...
{
    if (is_array($arr)) {
        $lang_path = array_pop(explode('/', $f));
        $name = explode('.', $lang_path)[0];
        if ($name != 'route') {
            $lang_arr = [];
            mac_extra_lang_set($arr, $name, $lang_arr);
            mac_set_extra_lang($lang_arr);
        }
...
/** leo 2022/03/01 陣列合併 **/
function mac_arr_merge(&$new, $old)
{
    foreach ($new as $key => &$val) {
        if (is_array($val) && array_key_exists($key, $old) && is_array($old[$key])) {
            mac_arr_merge($val, $old[$key]);
            $val = $val + $old[$key];
        }
    }
    $combin = $new + $old;
...
/* leo 2022/03/15 通知信息函式*/
function mac_send_notify($msg, $write_loag = false, $data = [])
{
    $admin_info = session("admin_info");
    $log_arr = [
        "管理員帳號 : " => $admin_info["admin_name"],
        "操作URL : " => $GLOBALS['config']['site']['site_url'],
        "信息 : " => $msg,
        "資料 : " => $data
    ];
    $msg = "Admin : " . $admin_info["admin_name"] . "\n Domain : " . $GLOBALS['config']['site']['site_url'] . "\n Msg : " . $msg;
    if ($write_loag != false) {
        trace($log_arr, $write_loag);
    }
    $data = [
        "text" => $msg,
        "chat_id" => config("notify_chat_id")
    ];
    if (config("notify_switch") || $write_loag == "notice") {
        mac_curl_post(config("notify_path"), $data);
    }
}
...
{
    $ch = @curl_init();
    $str_len = is_string($data) ? strlen($data) : 0;
    ...
    curl_setopt($ch, CURLINFO_CONTENT_LENGTH_UPLOAD, $str_len);
...
// CurlPUT数据提交-----------------------------------------
function mac_curl_put($url, $data, $heads = array(), $cookie = '')
{
    $ch = @curl_init();
    curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/45.0.2454.101 Safari/537.36');
    curl_setopt($ch, CURLOPT_URL, $url);
...
// leo Curl自定義数据加密提交-----------------------------------------
function mac_curl_ajax($method, $url, $data)
{
    //先進行數據型態設定與資料串加密
    $data = mac_json_aes_encode(json_encode(mac_format_data_type($data, $url)));
    //指定提交位置
    $url =  config('default_platform_api_url') . $url;
    $curl = @curl_init();
    ...
    curl_setopt($curl, CURLINFO_CONTENT_LENGTH_UPLOAD, strlen($data));
    ...
    */
    ...
    curl_close($curl);
    mac_send_notify($api_error_status);
    return $api_error_status; //显示返回信息
}

// leo 針對go api 提前調整送出data的參數型態-----------------------------------------
function mac_format_data_type($data, $tabel)
{
    $tabel = strtolower($tabel);
    $test = config('datatype');
    foreach ($data as $k => $v) {
        $test_type = $test[$tabel][$k];
        settype($data[$k], $test_type);
    }
    return $data; //显示返回信息
}
...
/** leo 2022/03/14 取得上傳資料夾內所有圖片檔案**/
function mac_upload_file_arr($dir)
{
    $fileArray = [];
    $files = scandir($dir);
    if (false !== $files) {
        foreach ($files as $file) {
            if ($file != '.' && $file != '..') {
                if (is_dir($dir . DIRECTORY_SEPARATOR . $file)) {
                    $fileArray[$file] = mac_upload_file_arr($dir . DIRECTORY_SEPARATOR . $file);
                } else {
                    $file_type = explode(".", $file)[1];
                    if (in_array($file_type, ['jpg', 'jpeg', 'png', 'gif', 'bmp'])) {
                        $dir_str = $dir . "/" . $file;
                        $dir_str = str_replace("\\", "/", $dir_str);
                        $dir_str = ltrim($dir_str, "./");
                        $fileArray[] = $dir_str;
                    }
                }
            }
...
/** leo 2022/03/14 多維轉一維函式**/
function mac_array_expand(&$new, $data = [], $index_replace = false)
{
    $data = empty($data) ? [] : $data;
    foreach ($new as $key => &$val) {
        if (is_array($val)) {
            $data = mac_array_expand($val, $data, $index_replace);
        } else {
            if ($index_replace) {
                $data[$key] = $val;
            } else {
...
/** leo 2021/12/28 限制字串檢查(僅英數可輸入) **/
function mac_not_only_num_en($str)
{
    $res = false;
    if (!preg_match("/^[a-zA-Z\d]*$/i", $str)) {
        $res = true;
    }
    return $res;
}
/** leo 2022/03/07 extra靜態檔套用語系函式 **/
...
/** leo 2022/03/07 設定extra靜態檔語系函式 **/
function mac_extra_lang_set(&$new, $path, &$extra_arr)
{
    foreach ($new as $key => &$val) {
        if (is_array($val)) {
            $path_a = $path . "/" . $key;
            mac_extra_lang_set($val, $path_a, $extra_arr);
            $val = $val;
        } else {
            $l_path = "extra/";
            $al_path = $l_path . $path . '/' . $key;
            $check = lang($al_path);
            if (strpos($check, "extra/") === false) {
                $extra_arr[$al_path] = $val;
                $val = $al_path;
            }
        }
    }
...
function mac_set_extra_lang($lang_arr)
{
    $s_lang = $GLOBALS['config']['app']['lang'];
    $s_lang_dir = APP_PATH . 'lang' . DS . $s_lang . '.php';
    $old_lang_arr = include $s_lang_dir;
    $file_lang_arr = mac_arr_merge($lang_arr, $old_lang_arr);
    $con = var_export($file_lang_arr, true);
...
function mac_url_img($url)
{
    if (substr($url, 0, 4) == 'mac:') {
        ...
        if ($GLOBALS['config']['upload']['mode'] == 'remote') {
            ...
            if (is_array($url)) {
                $url = MAC_PATH . $url[cookie('platform_lang')];
            } else {
                $url = MAC_PATH . $url;
            }
        }
...
/** leo 2021/12/28 修改路徑加載加密 改以openssl_加解密 原先加解密會有重複問題 **/
function mac_alphaID($in, $to_num = false, $pad_up = false, $passKey = '')
{
    ...
    $base = strlen($key);
    if ($to_num) {
        $out = url_b64decode($in);
        $out = openssl_decrypt($out, "aes-128-cbc", $key);
        // $in = strrev($in);
        // $out = 0;
        // $len = strlen($in) - 1;
        // for ($t = 0; $t <= $len; $t++) {
        ...
        $out = openssl_encrypt($in, "aes-128-cbc", $key);
        $out = url_b64encode($out);
        // if (is_numeric($pad_up)) {
        //     $pad_up--;
        //     if ($pad_up > 0) {
        //         $in += pow($base, $pad_up);
...
function mac_url($model, $param = [], $info = [])
{
    ...
    switch ($model) {
        ...
        case 'vod/type':
            $replace_to = [
                $info['type_id'], $info['type_en'], $param['page'],
...
function mac_label_topic_detail($param)
{
    ...
    $res["info"]["topic_vod_count"] = count(explode(",", $res["info"]["topic_rel_vod"]));
    return $res;
}
...
function mac_label_type($param)
{
    ...
    $where = ["type_id" => ["eq", (int)$param['id']]];
    $type_vod_count = model('Vod')->countData($where);
    $type_info["type_vod_count"] = $type_vod_count;
...
/** leo 2022/01/12 aes加密函式 **/
function mac_json_aes_encode(string $str, string $key = "", string $iv = "")
{
    $key = $key == "" ? config('aes_cbc_key') : $key;
    $iv = $iv == "" ? config('aes_cbc_iv') : $iv;
    $cipher = config('openssl_cipher');
    $encode_data = openssl_encrypt($str, $cipher, $key, 0, $iv);
    if ($encode_data === false) {
        trace($str);
    }
    return $encode_data;
}
/** leo 2022/01/12 aes解密函式 **/
function mac_json_aes_decode(string $str, string $key = "", string $iv = "")
{
    $key = $key == "" ? config('aes_cbc_key') : $key;
    $iv = $iv == "" ? config('aes_cbc_iv') : $iv;
    $cipher = config('openssl_cipher');
    $decode_data = openssl_decrypt($str, $cipher, $key, 0, $iv);
...
/* leo 2021/12/10 取得當前前台語系*/
function mac_get_platform_lang()
{
    $s_cookies = cookie('platform_lang') != '' ?  cookie('platform_lang') : $GLOBALS['config']['site']['lang'];
    return  $s_cookies;
}
/* leo 2021/12/23 config 撈取覆寫thinkphp 內的 helper.php 加入取config靜態檔時的語系判斷*/
function config($name = '', $value = null, $range = '')
{
    if (is_null($value) && is_string($name)) {
        if (0 === strpos($name, '?')) {
            $res = Config::has(substr($name, 1), $range);
        } else {
            $res = Config::get($name, $range);
            mac_extra_lang_get($res);
        }
        return $res;
    } else {
