...
class User extends Base
{
    ...
    {
        return $this->fetch('user/promote');
    }
    ...
    public function buy()
    {
        ...
        if (Request()->isPost()) {
            ...
            $group_day_list = [
                $GLOBALS['config']['site']['site_time_0'] => "site_time_0",
                $GLOBALS['config']['site']['site_time_1'] => "site_time_1",
                $GLOBALS['config']['site']['site_time_2'] => "site_time_2",
                $GLOBALS['config']['site']['site_time_3'] => "site_time_3"
            ];
            $pay_bonus_list = [
                1 => $GLOBALS['config']['user']['cash_bonus_1'],
                2 => $GLOBALS['config']['user']['cash_bonus_2'],
                3 => $GLOBALS['config']['user']['cash_bonus_3'],
                4 => $GLOBALS['config']['user']['cash_bonus_4'],
                5 => $GLOBALS['config']['user']['cash_bonus_5'],
                6 => $GLOBALS['config']['user']['cash_bonus_6'],
                7 => $GLOBALS['config']['user']['cash_bonus_7'],
                8 => $GLOBALS['config']['user']['cash_bonus_8'],
            ];
            if ($flag == 'card') {
                $card_no = htmlspecialchars(urldecode(trim($param['card_no'])));

                $res = model('Card')->useData($card_no, $GLOBALS['user']);
                return json($res);
            } elseif ($flag == "type_pay") {
                $price_type = input('param.type');
                if (!isset($pay_bonus_list[$price_type])) {
                    return json(['code' => 1001, 'msg' => lang('param_err')]);
                }
                $price = (float) $pay_bonus_list[$price_type];
                $data = [];
                $data['user_id'] = $GLOBALS['user']['user_id'];
                $data['order_code'] = 'pay' . mac_get_uniqid_code();
                $data['order_price'] = (float)$price;
                $data['order_time'] = (float)time();
                $data['order_remarks'] = isset($order_remarks) ? $order_remarks : "";
                $data['order_points'] = intval($GLOBALS['config']['user']['cash_ratio'] * $price);
                ...
                if (!empty($day) && !in_array($day, $group_day_list)) {
                    foreach ($group_day_list as $k => $v) {
                        if ($k == $day) {
                            $order_remarks = $v;
                        }
                    }
        ...
        $cash_ratio = $GLOBALS['config']['user']['cash_ratio'];
        $cash_to_point_1 = bcmul($GLOBALS['config']['user']['cash_bonus_1'], $cash_ratio);
        $cash_to_point_2 = bcmul($GLOBALS['config']['user']['cash_bonus_2'], $cash_ratio);
        $cash_to_point_3 = bcmul($GLOBALS['config']['user']['cash_bonus_3'], $cash_ratio);
        $cash_to_point_4 = bcmul($GLOBALS['config']['user']['cash_bonus_4'], $cash_ratio);
        $cash_to_point_5 = bcmul($GLOBALS['config']['user']['cash_bonus_5'], $cash_ratio);
        $cash_to_point_6 = bcmul($GLOBALS['config']['user']['cash_bonus_6'], $cash_ratio);
        $cash_to_point_7 = bcmul($GLOBALS['config']['user']['cash_bonus_7'], $cash_ratio);
        $cash_to_point_8 = bcmul($GLOBALS['config']['user']['cash_bonus_8'], $cash_ratio);
        $user_cash_bonus = [
            "bonus_1" => [
                "id" => 1,
                "cash" => $GLOBALS['config']['user']['cash_bonus_1'],
                "bonus" => $cash_to_point_1,
            ],
            "bonus_2" => [
                "id" => 2,
                "cash" => $GLOBALS['config']['user']['cash_bonus_2'],
                "bonus" => $cash_to_point_2,
            ],
            "bonus_3" => [
                "id" => 3,
                "cash" => $GLOBALS['config']['user']['cash_bonus_3'],
                "bonus" => $cash_to_point_3,
            ],
            "bonus_4" => [
                "id" => 4,
                "cash" => $GLOBALS['config']['user']['cash_bonus_4'],
                "bonus" => $cash_to_point_4,
            ],
            "bonus_5" => [
                "id" => 5,
                "cash" => $GLOBALS['config']['user']['cash_bonus_5'],
                "bonus" => $cash_to_point_5,
            ],
            "bonus_6" => [
                "id" => 6,
                "cash" => $GLOBALS['config']['user']['cash_bonus_6'],
                "bonus" => $cash_to_point_6,
            ],
            "bonus_7" => [
                "id" => 7,
                "cash" => $GLOBALS['config']['user']['cash_bonus_7'],
                "bonus" => $cash_to_point_7,
            ],
            "bonus_8" => [
                "id" => 8,
                "cash" => $GLOBALS['config']['user']['cash_bonus_8'],
                "bonus" => $cash_to_point_8,
            ],
        ];
        $this->assign('param', $param);
        $this->assign('config', $GLOBALS['config']['pay']);
        $this->assign('bonus_list', $user_cash_bonus);
    ...
    public function pay()
    {
        ...
        if ($res['info']['order_status'] == 1) {
            return $this->error(lang('index/order_payed'), 'user/index');
        }
        $this->assign('param', $param);
        $this->assign('config', $GLOBALS['config']['pay']);
    ...
    public function gopay()
    {
        ...
        } else if ($payment == 'zf777pay') {
            if ($payment_res == false) {
                return $this->error(lang('index/order_createfalse'), url('user/index'));
            } else {
                if (isset($payment_res['page_url'])) {
                    //使用777支付頁面
                    header("Location: " . $payment_res['page_url']);
                    exit;
                } else {
                    //使用本地頁面
                    $this->assign('payment', $payment_res);
                    return $this->fetch('user/payment_zf777pay');
                }
            }
        } else if ($payment == 'godpay') {
            if ($payment_res == false) {
                return $this->error(lang('index/order_createfalse'), url('user/index'));
            } else {
                if (isset($payment_res['page_url'])) {
                    ...
                    //使用本地頁面
                    $this->assign('payment', $payment_res);
                    return $this->fetch('user/payment_godpay');
                }
            }
        }
    }
    ...
    public function upgrade()
    {
        ...
        $group_day_list = [
            "site_time_0" => $GLOBALS['config']['site']["site_time_0"],
            "site_time_1" => $GLOBALS['config']['site']["site_time_1"],
            "site_time_2" => $GLOBALS['config']['site']["site_time_2"],
            "site_time_3" => $GLOBALS['config']['site']["site_time_3"],
        ];
        $this->assign('group_list', $group_list);
        $this->assign('group_day_list', $group_day_list);
        $this->assign('param', $param);
    ...
    public function plays()
    {
        ...
        $order = 'ulog_time desc';
        $res = model('Ulog')->listData($where, $order, $param['page'], $param['limit']);
        $res_2 = model('Ulog')->listData($where, $order, 0, 0);

        $this->assign('param', $param);
        $this->assign('list', $res['list']);
        $this->assign('all_list', $res_2['list']);
        $this->assign('all_count', count($res_2['list']));
    ...
    {
        $user = $GLOBALS['user'];
        $free_point = $GLOBALS['config']['user']['free_day_point'];
        $time = time();
        $data_day = (int) date("d", $user['user_free_point_time']);
        $now_day = (int) date("j");
        $new_point = bcadd($user['user_points'], $free_point);
        if ($now_day != $data_day) {
            $where = [];
            $where["user_id"] = ["eq", $user['user_id']];
            model('User')->fieldData($where, "user_points", $new_point);
            model('User')->fieldData($where, "user_free_point_time", $time);
            return json(lang('check_in_success'));
        } else {
            return json(lang('check_in_ready'));
        }
    }
}
