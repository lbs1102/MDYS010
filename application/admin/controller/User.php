...
class User extends Base
{
    ...
    public function info()
    {
        if (Request()->isPost()) {
            $param = input('post.');
            $now_time = time();
            if ($param['user_presentation_time'] != "" && !ctype_digit($param['user_presentation_time'])) {
                return $this->error(lang('param_err'));
            }
            if ($param['user_presentation_time'] == 0 || $param['user_presentation_time'] == "") {
                unset($param['user_presentation_time']);
            }
            if (isset($param['user_presentation_time'])) {
                $param['user_presentation_time'] = ((int)$param['user_presentation_time'] * 60 * 60 * 24);
            }
            if (isset($param['user_end_time']) && !is_numeric($param['user_end_time'])) {
                $param['user_end_time'] = strtotime($param['user_end_time']);
            }
            if (isset($param["user_id"]) && isset($param['user_presentation_time'])) {
                $where = [];
                $where['user_id'] = ['eq', $param["user_id"]];
                $res = model("User")->infoData($where);
                $old_user_presentation_time = $res["info"]['user_presentation_time'];
                $old_user_end_time = $res["info"]['user_end_time'];
                $add_user_presentation_time = $param['user_presentation_time'];
                if ($old_user_presentation_time > $now_time) {
                    $param['user_presentation_time'] = $old_user_presentation_time + $add_user_presentation_time;
                } else {
                    $param['user_presentation_time'] = $now_time + $add_user_presentation_time;
                }

                if (isset($param['user_end_time']) && $param['user_end_time']  > $param['user_presentation_time'] && $param['user_end_time'] > $now_time) {
                    $param['user_end_time'] = $param['user_end_time'] + $add_user_presentation_time;
                } else if ($old_user_end_time  > $param['user_presentation_time'] && $old_user_end_time > $now_time) {
                    $param['user_end_time'] = $old_user_end_time  + $add_user_presentation_time;
                } else {
                    $param['user_end_time'] =  $param['user_presentation_time'];
                }
                $param['group_id'] = "3";
            }
            $res = model('User')->saveData($param);
            if ($res['code'] > 1) {
                return $this->error($res['msg'], null, $param);
            }
            return $this->success($res['msg'], null, $param);
        }
