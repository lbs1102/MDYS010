...
class Annex extends Base
{
    ...
    public function check()
    {
        ...
        foreach (mac_array_expand($GLOBALS['config']) as $k => $v) {
            $pic_local_check = explode("/", $v);
            if ($pic_local_check[0] == "upload") {
                $local_pic[$pic_local_check[1]][] = $v;
            }
        }
        ...
        $vod_list = Db::name('Vod')->field(["vod_pic", "vod_pic_slide", "vod_pic_thumb", "vod_pic_screenshot"])->where(
            'vod_pic|vod_pic_slide|vod_pic_thumb|vod_pic_screenshot',
            'like',
            'upload%'
        )->fetchSql(false)->select();
        foreach ($vod_list as $k => $v) {
            foreach ($v as $k2 => $v2) {
                $pic_local_check = explode("/", $v2)[0];
                if ($pic_local_check == "upload") {
                    $local_pic['vod'][] = $v2;
                }
            }
        }
        $user_list = Db::name('User')->field(["user_portrait", "user_portrait_thumb"])->where(
            'user_portrait|user_portrait_thumb',
            'like',
            'upload%'
        )->fetchSql(false)->select();
        ...
        $topic_list = Db::name('Topic')->field(["topic_pic", "topic_pic_thumb", "topic_pic_slide"])->where(
            'topic_pic|topic_pic_thumb|topic_pic_slide',
            'like',
            'upload%'
        )->fetchSql(false)->select();
        ...
        $local_pic_arr = mac_array_expand(mac_upload_file_arr("./upload"));
        // $list = Db::name('Annex')->field('*')->where($where)->limit($limit_str)->fetchSql(false)->orderRaw('annex_time desc')->select();
        $list = Db::name('Annex')->field('*')->fetchSql(false)->orderRaw('annex_time desc')->select();
        $database_pic_arr = [];
        foreach ($list as $k3 => $v3) {
            $tmp = $v3['annex_file'];
            $database_pic_arr[] = $tmp;
            $url_path = explode("/", $tmp);
            $pic = './' . $tmp;
            switch ($url_path[1]) {
                case 'site':
                    if (!in_array($tmp, $local_pic["site"])) {
                        if (file_exists($pic)) {
                            unlink($pic);
                        }
                    }
                    break;
                case 'user':
                    if (!in_array($tmp, $local_pic["user"])) {
                        if (file_exists($pic)) {
                            unlink($pic);
                        }
                    }
                    break;
                case 'topic':
                    if (!in_array($tmp, $local_pic["topic"])) {
                        if (file_exists($pic)) {
                            unlink($pic);
                        }
                    }
        ...
        foreach (array_diff($local_pic_arr, $database_pic_arr) as $key => $value) {
            $pic = './' . $value;
            $url_path = explode("/", $value);
            switch ($url_path[1]) {
                case 'site':
                    if (!in_array($value, $local_pic["site"])) {
                        if (file_exists($pic)) {
                            unlink($pic);
                            mac_echo($value . '...del');
                        }
                    ...
                    break;
                case 'user':
                    if (!in_array($value, $local_pic["user"])) {
                        if (file_exists($pic)) {
                            unlink($pic);
                            mac_echo($value . '...del');
                        }
                    ...
                    break;
                case 'topic':
                    if (!in_array($value, $local_pic["topic"])) {
                        if (file_exists($pic)) {
                            unlink($pic);
                            mac_echo($value . '...del');
                        }
                    ...
                    break;
                case 'vod':
                    if (!in_array($value, $local_pic["vod"])) {
                        if (file_exists($pic)) {
                            unlink($pic);
                            mac_echo($value . '...del');
                        }
