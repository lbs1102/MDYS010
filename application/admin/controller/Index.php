...
class Index extends Base
{
    ...
    public function info()
    {
        if (Request()->isPost()) {
            $param = input('post.');
            if ($param["admin_pwd"] !== $param["admin_pwd_ck"]) {
                return $this->error("確認密碼錯誤");
            }
            if (mac_not_only_num_en($param["admin_pwd"])) {
                return $this->error(lang('validate/pass_special_characters'));
            }
            if (strlen($param["admin_pwd"]) < 6) {
                return $this->error(lang('validate/pass_length'));
            }
            ...
            $where = [];
            $where["admin_id"] = ["eq", $this->_admin["admin_id"]];
            $res = model('Admin')->fieldData($where, "admin_pwd", md5($param["admin_pwd"]));
            if ($res['code'] > 1) {
                return $this->error($res['msg']);
            }
            return $this->success($res['msg']);
        }
        return $this->fetch('admin@index/info');
    }
